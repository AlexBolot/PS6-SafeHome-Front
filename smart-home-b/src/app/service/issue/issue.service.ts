...
export class IssueService {
  ...
  getDeclaredBy(id: number): Observable<Issue[]> {
    return this.httpClient.get<Issue[]>(this.API_url).map(issues => issues.filter(issue => issue.IDAuthor === id).sort((issue1, issue2): number => {
      if (issue1.DeclarationDate < issue2.DeclarationDate) { return 1; }
      if (issue1.DeclarationDate > issue2.DeclarationDate) { return -1; }
        return 0;
        }));
  }
  ...
  getAssignedTo(id: number): Observable<Issue[]> {
    return this.httpClient.get<Issue[]>(this.API_url)
      ...
      if (issue1.IDUrgency < issue2.IDUrgency) { return 1; }
      if (issue1.IDUrgency > issue2.IDUrgency) { return -1; }
      return 0;
    }));
  }
